/* Home screen. */
const { Button: SButton, Card: SCard, Stat: SStat, Badge: SBadge } = window.SalvoInnovationDesignSystem_cdc9ce;

function HeroHome({ onNavigate }) {
  return (
    <section style={{ position: "relative", overflow: "hidden", background: "var(--paper)" }}>
      <img src="../../assets/brand-flags-salvo.svg" alt="" style={{ position: "absolute", right: 16, top: 8, width: "min(740px, 76vw)", opacity: 0.97, pointerEvents: "none" }} />
      <Container style={{ position: "relative", padding: "clamp(104px,13vw,156px) clamp(20px,5vw,56px) clamp(48px,7vw,90px)" }}>
        <div style={{ maxWidth: 860 }}>
          ...
          <div style={{ marginTop: 34, display: "flex", gap: 14, flexWrap: "wrap" }}>
            <SButton variant="primary" size="lg" onClick={() => onNavigate("contact")} iconRight={<Icon name="arrow-right" size={18} />}>Book a salvo</SButton>
            <SButton variant="secondary" size="lg" onClick={() => onNavigate("workshops")}>See the formats</SButton>
          </div>
        </div>
      </Container>
...
function WhatWeDo({ onNavigate }) {
  const offers = [
    { icon: "compass", tag: "Consulting", title: "Digital product consulting", body: "Strategy, discovery and direction for products that have lost their bearings. We help you find the next right thing to build — and the confidence to ship it.", points: ["Product strategy & roadmaps", "Discovery & user research", "Embedded with your team"], tone: "default" },
    { icon: "zap", tag: "Facilitation", title: "Workshop design & facilitation", body: "High-energy, decision-forward workshops for leadership and product teams. We design the session, run the room, and hand you outcomes — not homework.", points: ["Product sprints + a tested prototype", "Strategy & alignment days", "Remote, in-person or hybrid"], tone: "ink" },
  ];
  return (
    <section style={{ padding: "clamp(64px,9vw,112px) 0" }}>
      <Container>
        ...
        <div style={{ marginTop: 48, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 22 }}>
          {offers.map((o) => (
            <SCard key={o.tag} variant="sticker" tone={o.tone} interactive style={{ padding: 34 }}>
              <div style={{ display: "inline-flex", width: 56, height: 56, borderRadius: 14, alignItems: "center", justifyContent: "center", background: o.tone === "ink" ? "var(--signal-500)" : "var(--ink-900)", marginBottom: 22 }}>
                <Icon name={o.icon} size={28} color={o.tone === "ink" ? "#fff" : "var(--volt-500)"} />
              </div>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: o.tone === "ink" ? "var(--volt-500)" : "var(--signal-600)", marginBottom: 10 }}>{o.tag}</div>
              <h3 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 28, lineHeight: 1.05, letterSpacing: "-0.02em", margin: 0, color: o.tone === "ink" ? "var(--paper)" : "var(--ink-900)" }}>{o.title}</h3>
              <p style={{ marginTop: 14, fontSize: 16.5, lineHeight: 1.55, color: o.tone === "ink" ? "var(--text-on-dark-muted)" : "var(--text-secondary)" }}>{o.body}</p>
              <ul style={{ listStyle: "none", margin: "20px 0 26px", padding: 0, display: "flex", flexDirection: "column", gap: 11 }}>
                {o.points.map((p) => (
                  <li key={p} style={{ display: "flex", gap: 10, alignItems: "center", fontSize: 15.5, fontWeight: 600, color: o.tone === "ink" ? "var(--paper)" : "var(--ink-900)" }}>
                    <Icon name="check" size={18} color="var(--signal-500)" stroke={3} />{p}
                  </li>
                ))}
              </ul>
              <SButton variant={o.tone === "ink" ? "volt" : "ink"} size="md" onClick={() => onNavigate(o.tag === "Facilitation" ? "workshops" : "services")} iconRight={<Icon name="arrow-up-right" size={16} />}>Explore {o.tag.toLowerCase()}</SButton>
            </SCard>
          ))}
        </div>
      </Container>
...
function StatsBand() {
  return (
    <section style={{ background: "var(--ink-900)", color: "var(--paper)", position: "relative", overflow: "hidden" }}>
      <img src="../../assets/motif-ripple.svg" alt="" style={{ position: "absolute", right: -60, top: -60, width: 320, opacity: 0.14, filter: "invert(1)" }} />
      <Container style={{ padding: "clamp(56px,8vw,96px) clamp(20px,5vw,56px)", position: "relative" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px,1fr))", gap: 32 }}>
          <SStat tone="paper" value="Days" label="Idea → tested prototype" />
          <SStat tone="paper" value="120+" label="Workshops facilitated" />
          <SStat tone="paper" value="40+" label="Teams unstuck" />
          <SStat tone="paper" value="9/10" label="Would run another" />
        </div>
      </Container>
    </section>
  );
...
function Testimonial({ onNavigate }) {
  return (
    <section style={{ padding: "clamp(64px,9vw,112px) 0" }}>
      <Container size="narrow" style={{ textAlign: "center" }}>
        <Icon name="quote" size={44} color="var(--signal-500)" />
